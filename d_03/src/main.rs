...
fn find_complete_number(data: &[&str], position: (usize, usize)) -> usize {
    ...
    if position.1 == data[position.0].len() - 1 {
        center = -2;
    }
    if position.1 == 0 {
        center = 0;
    ...
    for i in center..=center + 2 {
        if data[position.0]
            .chars()
            .nth(position.1.wrapping_add_signed(i))
            .unwrap()
            .is_numeric()
        {
            num_as_str.push(
...
fn find_nums_around_symbol(
    ...
    for mut row_mod in -1..=1 {
        for mut column_mod in -1..=1 {
            if position.0 + row_mod < 0 || position.0 + row_mod >= data.len() as isize {
                row_mod = 0;
            }
            let row = (position.0 + row_mod) as usize;
            if position.1 + column_mod < 0 || position.1 + column_mod >= data[row].len() as isize {
                column_mod = 0;
            }
...
fn main() {
    let data_input: Vec<&str> = include_str!("input.txt").trim_end().split('\n').collect();
